...
description: Learn about Java Classes and Objects. Understand the concept of OOP, how to create classes, instantiate objects, and access members.
icon: Box
---
...
<Quiz
  question="A class is a _____ for the object."
  options={["Copy", "Blueprint", "Instance", "Variable"]}
...
classDiagram
    ...
    note for Car "Class (Blueprint)"

    class MyTesla {
        color = "Red"
        model = "Model S"
    }
    note for MyTesla "Object (Instance)"

    Car <|-- MyTesla : Instance of
```
...
**Tip 💡:** Remember: **Class** = Blueprint (Plan), **Object** = House (Real thing). You can't live in a blueprint!
...
<Quiz
  question="Which operator is used to access the members (fields and methods) of an object?"
  options={["-> (arrow)", ". (dot)", ": (colon)", "# (hash)"]}
  correctAnswerIndex={1}
/>
...
<Challenge
  id="class-objects"
  nextChapterId="methods"
  question="Create a class named 'Dog' with a method 'bark' that prints 'Woof'. Create an object and call the method."
  expectedOutput="Woof"
  hint="Define a class 'Dog' with a void method 'bark()'. In main, create an object: Dog d = new Dog();"
  solution={`class Dog {
    void bark() {
...
*   **Object**: The actual instance created from the blueprint (e.g., `myTesla`).
*   **Members**: Fields (state) and Methods (behavior).

## Common Pitfalls

> [!WARNING]
> **Null Pointer**: If you declare `Dog d;` but don't do `d = new Dog();`, `d` is `null`. Calling `d.bark()` will crash your program with a `NullPointerException`.

> [!WARNING]
> **Class vs Object**: Remember, you can't drive the blueprint of a car. You need to create an object first!

## What's Next?

Objects have behavior. Let's learn how to define that behavior using methods.
[Master Java Methods →](/docs/methods)
