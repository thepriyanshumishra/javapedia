...
<Quiz
  ...
  options={["void", "int", "class type", "No return type"]}
...
**Tip 💡:** A constructor is a special method that has the **EXACT** same name as the class and **NO** return type (not even void!).
...
<Quiz
  question="When is a default constructor created automatically?"
  options={[
    "Always",
    "When no constructor is defined",
    "When a private constructor is defined",
    "When a parameterized constructor is defined",
  ]}
...
## 5. Constructors Overloading in Java
...
It is also possible to call one constructor from another constructor using `this()`. This is known as constructor chaining.
...
<Challenge
  id="constructors"
  nextChapterId="static-keyword"
  question="Create a class 'Car' with a constructor that takes a 'model' name. Create an object with model 'Tesla' and print it."
  expectedOutput="Tesla"
  hint="Use 'this.model = m' inside the constructor to assign the value."
  solution={`class Car {
    String model;
    Car(String m) {
        this.model = m;
...
*   **No Return**: Does not have a return type (not even `void`).
*   **Purpose**: Used to initialize objects.

## Common Pitfalls

> [!WARNING]
> **The Void Trap**: If you write `void Car() { ... }`, Java thinks it's just a regular method named "Car", not a constructor!
>
> **Default Constructor**: If you define _any_ constructor, Java stops providing the default no-arg constructor. You have to add it back manually if you need it.

## What's Next?

Who is allowed to touch your data? Let's protect our objects.
[Understand Access Modifiers →](/docs/access-modifiers)
