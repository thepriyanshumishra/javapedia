...
**Tip 💡:** Exceptions are like "Safety Nets" 🕸️. Without them, your program crashes (falls). With them, you catch the error and keep going!
...
<Quiz
  question="Which block is used to enclose code that might throw an exception?"
...
You can also create unchecked exceptions by extending `RuntimeException`.
...
// No need to declare 'throws' in method signature.
```
...
<Challenge
  ...
  question="Handle the ArrayIndexOutOfBoundsException when accessing index 5 of an array of size 3. Print 'Out of bounds'."
  expectedOutput="Out of bounds"
  hint="Wrap the code in a try-catch block and catch ArrayIndexOutOfBoundsException."
...
- **Try-Catch-Finally**: The standard block for handling errors.
- **Checked vs Unchecked**: Checked exceptions (like `IOException`) MUST be handled. Unchecked (like `NullPointerException`) are usually bugs.
- **Throw vs Throws**: `throw` triggers an exception; `throws` declares that a method _might_ trigger one.

## Common Pitfalls

> [!WARNING]
> **Empty Catch**: Never leave a catch block empty! You won't know if something went wrong.
>
> **Catching Exception**: Avoid catching the generic `Exception` class unless necessary. It catches _everything_, even bugs you didn't expect.

## What's Next?

Now that we can handle errors, let's learn how to handle groups of objects.
[Learn Collections Framework →](/docs/collections-overview)
