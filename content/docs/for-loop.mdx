...
<Quiz
  question="Which loop is best suited for iterating over arrays?"
  options={["while loop", "do...while loop", "for-each loop", "infinite loop"]}
  correctAnswerIndex={2}
/>
...
- **For-Each**: A cleaner syntax for iterating over arrays/collections: `for (Type var : array)`.
...
What if we don't know how many times we need to loop?
[Learn While Loops →](/docs/while-and-do-while-loop)
