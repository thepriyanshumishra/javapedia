...
Here, `Dog` is a subclass and `Animal` is a superclass. The `Dog` class inherits the methods and fields of the `Animal` class.
...
<div className="text-muted-foreground mt-2 text-center text-sm">
  Class Diagram: Dog inherits from Animal
</div>
...
**Tip 💡:** Inheritance is an **"IS-A"** relationship. A Dog **IS-A** Animal. If it doesn't sound right (e.g., A Dog IS-A Car), don't inherit!

</Callout>

## IS-A vs HAS-A Relationship

*   **IS-A Relationship (Inheritance)**: Represents a hierarchy.
    *   Example: `Car` IS-A `Vehicle`.
    *   Implemented using `extends`.
*   **HAS-A Relationship (Association/Aggregation)**: Represents usage.
    *   Example: `Car` HAS-A `Engine`.
    *   Implemented by creating an instance of a class inside another class.
...
<Challenge
  id="inheritance"
  nextChapterId="polymorphism"
  question="Create a class 'Dog' that extends 'Animal'. 'Animal' has a method 'eat()'. Call 'eat()' using a 'Dog' object."
  expectedOutput="Eating..."
  hint="Use the 'extends' keyword to inherit from the Animal class."
...
- **Super**: Refers to the parent class (used for constructors or methods).
- **Single Inheritance**: A class can only extend ONE parent class.

## Common Pitfalls

> [!WARNING]
> **Private Members**: Child classes do NOT inherit private members of the parent.
>
> **Cyclic Inheritance**: Class A cannot extend Class B if Class B extends Class A.

## What's Next?

If a Dog is an Animal, can we treat a Dog object as an Animal variable?
[Learn Polymorphism →](/docs/polymorphism)
