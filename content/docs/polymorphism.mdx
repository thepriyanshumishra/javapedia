...
**Tip 💡:** Polymorphism is like a universal remote. You press "Power" (method), and it works on TV, DVD, or Stereo (objects), but each reacts differently!
...
Since Java 5.0, it is possible to override a method by changing the return type if the subclass return type is a subclass of the parent return type. This is known as a **Covariant Return Type**.
...
class Dog extends Animal {
    @Override
    Dog get() { // Covariant return type (Dog is a subclass of Animal)
...
<Quiz
  question="Polymorphism allows us to perform a single action in..."
  options={["Only one way", "Different ways", "Reverse order", "Random ways"]}
  correctAnswerIndex={1}
/>

Here, the `render()` method behaves differently for different classes.
...
<div className="text-muted-foreground mt-2 text-center text-sm">
  Class Diagram: Polymorphic Shape Hierarchy
</div>
...
<Challenge
  id="polymorphism"
  nextChapterId="interfaces"
  question="Create a class 'Shape' with method 'draw()'. Override it in 'Circle' to print 'Circle'. Call draw() on a Shape reference holding a Circle."
  expectedOutput="Circle"
  hint="Override the 'draw' method in the Circle class to provide specific implementation."
...
- **Overriding**: Same method signature in child class (Run-time).
- **Dynamic Dispatch**: Java decides which method to call based on the actual object, not the reference type.

## Common Pitfalls

> [!WARNING]
> **Static Methods**: You cannot override static methods. If you try, it's called "Method Hiding", and it behaves differently.
>
> **Missing Annotation**: Always use `@Override`. It stops you from making typos (like `tostring()` instead of `toString()`).

## What's Next?

How do we protect our data from being messed up by other classes?
[Learn Encapsulation →](/docs/encapsulation)
