...
<Quiz
  question="Which class is used to create strings in Java?"
  options={["Text", "String", "Char", "Word"]}
  correctAnswerIndex={1}
/>

Here, we have created a string variable named `type`.hat holds the value "Hello, World!".
...
<Quiz
  question="What is the output of 'Java'.substring(1, 3)?"
...
<Challenge
  id="strings"
  nextChapterId="class-objects"
  question="Create a string 'Java Programming' and print its length."
  expectedOutput="16"
  hint="Use the .length() method to find the length of a string."
...
- **Immutable**: Strings cannot be changed. Methods like `.toUpperCase()` return a _new_ string.
...
> [!WARNING]
> **The `==` Trap**: `s1 == s2` checks if they are the _same object in memory_, not if they have the same text. Use `s1.equals(s2)`.
>
> **Concatenation in Loops**: Using `+` in a loop (e.g., `s += "a"`) is very slow because it creates a new String object every time. Use `StringBuilder` instead.

## What's Next?

We've covered the basics! Now, let's dive into the heart of Java: Object-Oriented Programming.
[Start OOP: Classes & Objects →](/docs/class-objects)
