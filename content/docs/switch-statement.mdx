...
- If `expression` matches `value1`, the code of `case value1` is executed. Similarly, the code of `case value2` is executed if `expression` matches `value2`.
- If there is no match, the code of the **default case** is executed.

<Callout>

**Note:** The working of the switch-case statement is similar to the [Java if...else...if ladder](/docs/if-else-statement). However, the syntax of the `switch` statement is cleaner and much easier to read and write.
...
Here, the `size` variable is assigned the value `Large`.
...
The `break` statement is used to terminate the **switch-case** statement. If `break` is not used, all the cases after the matching case are also executed (fall-through). For example,
...
In the above example, `expression` matches `case 2`. Here, we haven't used the `break` statement after each case.

Hence, all the cases after `case 2` are also executed.

This is why the `break` statement is needed to terminate the **switch-case** statement after the matching case. To learn more, visit [Java break Statement](/docs/break-statement).
...
In the above example, we created a **switch-case** statement. Here, the value of `expression` doesn't match any of the cases.
