...
Here, `this` is a reference variable that refers to the current object. In Java, the `this` keyword is used within a class to refer to the current instance (object) of that class. It’s commonly used in methods or constructors to reference the current object’s properties or to call other constructors within the same class.
...
**Tip 💡:** `this` is like saying "MY" variable. `this.name` means "MY name", not the name you just passed me!
...
When instance variables and parameters have the same name, using `this` helps avoid ambiguity:
...
Here, `this()` helps manage multiple constructors, improving readability and reducing code duplication. However, it should be used carefully.
...
<Quiz
  ...
  options={["Yes", "No", "Only for static variables", "Only in main method"]}
  correctAnswerIndex={1}
/>
...
<Challenge
  id="this-keyword"
  nextChapterId="final-keyword"
  question="Use 'this' to assign the parameter 'id' to the instance variable 'id'. Print 'id'."
  expectedOutput="101"
  hint="Use 'this.id = id' to distinguish the instance variable from the parameter."
...
- **Reference**: `this` refers to the _current_ object instance.
- **Disambiguation**: `this.name = name` separates the field from the parameter.
- **Constructor Chaining**: `this()` calls another constructor in the same class.

## Common Pitfalls

> [!WARNING]
> **Static Context**: You cannot use `this` inside a `static` method (like `main`), because static methods don't belong to an instance.

## What's Next?

We know how to change variables. But how do we stop them from changing?
[Learn Final Keyword →](/docs/final-keyword)
