...
1.  **Annual Compounding**: Set the compounding frequency to 1. How does it compare to Simple Interest?
2.  **Double the Money**: Experiment with inputs to find how long it takes to double your money at 10% interest.
3.  **Math.pow**: What does `Math.pow(2, 3)` return? Write a small test program to check.
4.  **Rounding**: The output has many decimal places. Can you format it to 2 decimal places using `System.out.printf`?
5.  **Brain Twister**: Calculate the difference between Simple Interest and Compound Interest for the same Principal, Rate, and Time.
