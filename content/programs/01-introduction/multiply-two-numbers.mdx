...
_Multiplication can be performed between positive, negative numbers as well._
...
1.  **Three Numbers**: Modify the program to multiply three numbers.
2.  **Zero**: What happens if you multiply any number by 0?
3.  **Negative**: Multiply a positive number by a negative number. Is the result negative?
4.  **Mixed Types**: Try multiplying an `int` by a `double`. What is the result type?
5.  **Brain Twister**: Calculate `100000 * 100000` using `int` variables. Does it print the correct value? If not, why? (Hint: `int` range).
