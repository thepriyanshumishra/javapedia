...
1.  **Swap Doubles**: Create a program to swap two `double` values using a temporary variable.
2.  **Swap Three**: Can you swap three numbers? `a` -> `b`, `b` -> `c`, `c` -> `a`.
3.  **Character Swap**: Try swapping two `char` variables (e.g., 'A' and 'B').
4.  **Overflow Check**: Use the arithmetic method with `a = 2000000000` and `b = 2000000000`. Does it work correctly?
5.  **Brain Twister**: Can you swap two numbers in a single line of code?
    *Hint: `b = a + b - (a = b);`*
