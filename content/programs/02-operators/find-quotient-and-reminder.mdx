...
1.  **Division by Zero**: What happens if you enter `0` as the divisor? Try it.
2.  **Negative Numbers**: Try dividing `-10` by `3`. What are the quotient and remainder?
3.  **Floating Point**: Change the variables to `double`. Does the modulus operator still work?
4.  **Last Digit**: How can you use modulus to find the last digit of any number? (Hint: `n % 10`).
5.  **Brain Twister**: Can you find the quotient *without* using the `/` operator? (Hint: Repeated subtraction).
